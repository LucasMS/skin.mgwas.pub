...
files <- list.files(d.in, pattern = "*.all.tsv") %>% 
  paste0(d.in, .)
res <- NULL
for (i in files){
...
```{r, fig.width= 12}
m.counts <- counts %>% 
  select(Gene_name, names, count) %>%
  pivot_wider(names_from = names, values_from = count) %>% 
  left_join(res) %>% 
  left_join(gene_classification) %>% 
  mutate(Classification = if_else(is.na(Classification),
                                  "Others",
...
rownames(m.counts) <- m.counts$Gene_name

pca_res <- prcomp(m.counts %>% select(starts_with("Cont")), scale. = TRUE)
library(ggfortify)
autoplot(pca_res, data = m.counts, colour = "Classification", label = TRUE) 



m.counts.matrix <- m.counts %>% select(starts_with("Cont"),
                    Gene_name) %>%
  column_to_rownames("Gene_name")
  
m.counts.matrix <- as.matrix(m.counts.matrix)

pheatmap::pheatmap(m.counts.matrix,cluster_cols = F, scale = "column")
...
counts %>% 
  ...
  geom_point(aes(col = significant, shape = Colonization, group = Colonization)) +
  ggpubr::rotate_x_text()
```
