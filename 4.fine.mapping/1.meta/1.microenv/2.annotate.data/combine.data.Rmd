...
# Optimize with fread
afq <- fread("~/IKMB/projects/skin.mgwas/results/1.data.harmonization/2.genotype/kora.popgen.maf/PopGen_and_KORA.harmonized.all.MAF.frqx") %>% 
  magrittr::set_colnames(make.names(colnames(.)))
```
# Remove Species-level results
Done after Nat Comm reviewer comments 

 - meta
```{r}
meta <- meta %>% 
  #remove species results
...
```
 ...
 - gene files
```{r, warning=F}
genes.files <- genes.files %>% 
  data.frame(files = .) %>% 
  separate(files, c("roundid", "meta.id", "type", "metanalysis"), remove = F) %>% 
  mutate(meta.id = str_remove(meta.id, "meta") %>% 
           as.integer(),
         type = str_remove(type, "type"),
         metanalysis = str_remove(metanalysis, "metanalysis")) %>% 
  inner_join(meta %>% select(metanalysis, type, meta.id) %>% unique(), 
             by = c("meta.id", "type", "metanalysis")) %>% 
  pull(files)
```
...
## March Gtex with snps

```{r}
egenes <- egenes %>% filter(refsnp_id %in% snps$refsnp_id)
```
