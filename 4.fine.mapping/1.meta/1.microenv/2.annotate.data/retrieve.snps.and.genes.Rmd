...
## Import data
```{r}
d.in <- "~/IKMB/projects/skin.mgwas/results/4.fine.mapping/1.meta/1.microenv/1.analysis/1.analysis"
d.in.list <- d.in %>% 
...
folder.id <- d.in.list %>% 
  data.frame(d.in = .) %>% 
  separate(d.in, c(c(1:12) %>% as.character(), "folder"), sep = "/") %>% 
  pull(folder) %>% 
